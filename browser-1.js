...
const KEYWORDS = [
  {
    keyword: "DEPO89",
    domain: "https://elshehabfoundation.com/women-programe/",
  },
  {
    keyword: "DEPO89 LOGIN",
    domain: "https://elshehabfoundation.com/women-programe/",
  },
  {
    keyword: "DEPO89 DAFTAR",
    domain: "https://elshehabfoundation.com/women-programe/",
  },
  {
    keyword: "DEPO89 LINK ALTERNATIF",
    domain: "https://elshehabfoundation.com/women-programe/",
  },
  {
    keyword: "DEPO89 RTP",
  ...
  {
    keyword: "SITUS DEPO89",
    domain: "https://elshehabfoundation.com/women-programe/",
  },
  {
    keyword: "DEPO89 RESMI",
    domain: "https://elshehabfoundation.com/women-programe/",
  },
  {
    keyword: "DEPO89 SLOT",
...
async function visitKeyword({ keyword, domain }) {
  writeLog(`Visiting keyword: ${keyword} -> ${domain}`);
  let browser;
  const timestamp = new Date().toISOString();

  try {
    writeLog(`Assuming VPN (UrbanVPN) is already active.`);
    ...
    // Decide device type (50% desktop / 50% mobile)
    const isMobile = Math.random() < 0.5;
    let deviceType = isMobile ? "mobile" : "desktop";

    if (isMobile) {
      // Advanced: pick a mobile profile and apply emulation
      const profile = pickRandom(MOBILE_PROFILES);
      await page.setUserAgent(profile.userAgent);
      await page.setViewport(profile.viewport);
      // enable touch emulation if puppeteer supports it
      try {
        await page.emulateMediaFeatures([]); // noop placeholder: leaving for compatibility
      } catch (e) {
        /* ignore */
      }
      writeLog(`Using mobile profile: ${profile.name}`);
    } else {
      // Desktop
      await page.setUserAgent(pickRandom(UA_LIST_DESKTOP));
      await page.setViewport({ width: 1366, height: 768 });
      writeLog("Using desktop profile");
    }
    ...
    await scrollGoogleToTarget(page, domain);

    const searchScreenshot = await saveScreenshot(
      page,
      "google-search-highlighted",
      keyword
    );
    if (searchScreenshot)
      await sendTelegramMessage(
        `Google search for ${keyword} (${deviceType})`,
        path.join(SCREENSHOT_DIR, searchScreenshot)
      );
    updateHTMLReport({
      keyword,
      url: searchUrl,
      status: "Google search visited",
    ...
    // Click target link if available
    const links = await page.$$eval("a", (anchors) =>
      anchors.map((a) => ({ href: a.href, text: a.innerText }))
    );
    const targetLink = links.find((l) => l.href && l.href.includes(domain));
    if (targetLink) {
      ...
      const preClickScreenshot = await saveScreenshot(
        page,
        "before-click",
        keyword
      );
      if (preClickScreenshot)
        await sendTelegramMessage(
          `Before clicking link (${deviceType})`,
          path.join(SCREENSHOT_DIR, preClickScreenshot)
        );
      updateHTMLReport({
        keyword,
        url: targetLink.href,
        status: "Before click",
        screenshot: preClickScreenshot,
        timestamp,
      });

      try {
        await page
          .click(`a[href="${targetLink.href}"]`, { button: "left", delay: 100 })
          .catch(async () => {
            await page
              .goto(targetLink.href, { waitUntil: "networkidle2" })
              .catch(() => {});
          });
      } catch (e) {
        // fallback to goto if click fails
        try {
      ...
      const landingScreenshot = await saveScreenshot(
        page,
        "clicked-link",
        keyword
      );
      if (landingScreenshot)
        await sendTelegramMessage(
          `Clicked link (${deviceType})`,
          path.join(SCREENSHOT_DIR, landingScreenshot)
        );
      updateHTMLReport({
        keyword,
        url: targetLink.href,
      ...
      // Try to simulate analytics on the landing page
      await simulateAnalytics(page, targetLink.href, deviceType).catch(
        () => {}
      );
    }

    // Visit landing page directly (ensures direct landing)
    try {
      await page.goto(domain, { waitUntil: "networkidle2" });
    } catch (e) {
      writeLog(`Direct goto failed: ${e.message || e}`);
    }
    const landingScreenshot2 = await saveScreenshot(
      page,
      "landing-page",
      keyword
    );
    if (landingScreenshot2)
      await sendTelegramMessage(
        `Visited landing page (${deviceType})`,
        path.join(SCREENSHOT_DIR, landingScreenshot2)
      );
    updateHTMLReport({
      keyword,
      url: domain,
      status: "Landing page visited",
      screenshot: landingScreenshot2,
      timestamp,
    });

    // Simulate GA events on the landing page
    await simulateAnalytics(page, domain, deviceType).catch(() => {});

    // Scroll the page (simulate reading)
    await scrollPage(page, SCROLL_MS);
    const scrolledScreenshot = await saveScreenshot(page, "scrolled", keyword);
    if (scrolledScreenshot)
      await sendTelegramMessage(
        `Scrolled page (${deviceType})`,
        path.join(SCREENSHOT_DIR, scrolledScreenshot)
      );
    updateHTMLReport({
      keyword,
      url: domain,
      status: "Page scrolled",
      screenshot: scrolledScreenshot,
      timestamp,
    });

    // Wait some "engagement" time then send a final analytics pulse
    await randomDelay(2000, 4000);
    await simulateAnalytics(page, domain, deviceType).catch(() => {});

    await browser.close();
  } catch (err) {
    writeLog(`Error visiting ${keyword}: ${err.message || err}`);
    updateHTMLReport({
      keyword,
      url: domain,
